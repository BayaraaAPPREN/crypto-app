    ...
    <div className='rounded-div flex items-center justify-between h-20 font-bold'>
        ...
        {user?.email ? (
            <div>
                ...
                <button className='mx-4' onClick={handleSignOut}>Sign out</button>
            </div>
        ) : (
            <div className='hidden md:block'>
            <Link to='/signin' className='p-4 hover:text-accent'>Sign In</Link>
            <Link to='/signup' className='bg-button text-btnText px-5 py-2 ml-2 rounded-lg shadow-lg hover:shadow-2xl'>Sign Up</Link>
            </div>
        ) }
